...
INPUT= $(wildcard *.cpp)
SOURCE= $(INPUT) include/InitShader.cpp
...
# Linux (default)
ifeq ($(OS),Windows_NT)
	LDFLAGS = -lopengl32 -lfreeglut -lglew32 -Wl,--subsystem,windows
else
	LDFLAGS = -L/usr/lib/nvidia-331/ -lGL -lglut -lGLEW -lGLU -lXmu -lXext -lX11 -lXi -lm -L./lib/ -lglui
endif
...
ifeq ($(OS),Windows_NT)
clean:
	rm -f $(OBJECT) depend $(EXECUTABLE).exe
else
clean:
